...
WHILE @VAR_CONTRAT_EN_COURS<=@VAR_TOTAL_CONTRAT --boucle par contrat
	...
	SELECT @VAR_CEMP_SALARIE=IND_CEMP FROM VENTILATION_INDIVIDUS WHERE IND_MATRICULEHRU=@VAR_CMATR;--récupération de l'employeur du salarié
	SELECT @VAR_PCI_SIREN=IND_SIREN FROM VENTILATION_INDIVIDUS WHERE IND_MATRICULEHRU=@VAR_CMATR;--récupération du SIREN
	SELECT @VAR_PCI_SALARIE=IND_MATRICULEHRS FROM VENTILATION_INDIVIDUS WHERE IND_MATRICULEHRU=@VAR_CMATR AND IND_CEMP=@VAR_CEMP_SALARIE;--récupération du matricule HRS 
	SELECT @VAR_STATUT_MIGRATION=IND_STATUS FROM VENTILATION_INDIVIDUS WHERE IND_MATRICULEHRU=@VAR_CMATR;--récupération du statut de la migration
	SELECT @VAR_PCI_ORDRE_TEMPORAIRE=COUNT(*) FROM HR_SPRINT_CONTRATTRAVAIL WHERE PCI_SALARIE=@VAR_PCI_SALARIE; --recherche du nombre de contrat salarié

...
SELECT @VAR_PCI_SIREN=Siren FROM EMPLOYEUR WHERE SOCIETE_LIGNE=@VAR_SOCIETE_LIGNE;--récupération du SIREN de l'employeur
	...
	SELECT @VAR_STATUT_MIGRATION=IND_STATUS FROM VENTILATION_INDIVIDUS WHERE IND_MATRICULEHRU=@VAR_CMATR;--récupération du statut de la migration
		...
		@VAR_PCI_TYPEINSEE,@VAR_PCI_CODEINSEE,@VAR_PCI_ANCIENNUM,@VAR_PCI_PASEXCLU,@VAR_PCI_DNACODUNITEHOR,@VAR_PCI_SALARIEREMPL);
...
SELECT @VAR_PRE_ORDRE=SUM(PRE_ORDRE+1) FROM HR_SPRINT_RETENUESALAIRE WHERE PRE_SALARIE=@VAR_PRE_SALARIE AND PRE_SIREN=@VAR_PRE_SIREN;
IF @VAR_PRE_ORDRE IS NULL
SET @VAR_PRE_ORDRE=1;
...
SELECT @VAR_PRE_MONTANTTOT=SUM(MTADD-MTSUB) FROM SAISIEARRET WHERE SAISIEARRET_LIGNE=@VAR_SAISIEARRET_EN_COURS;--récupération du solde de la saisie arret
...
VALUES (@VAR_PRE_SIREN,@VAR_PRE_SALARIE,@VAR_PRE_ORDRE,@VAR_PRE_LIBELLE,@VAR_PRE_DATEDEBUT,@VAR_PRE_DATEFIN,@VAR_PRE_MONTANTTOT,@VAR_PRE_ACTIF,@VAR_PRE_RETENUESAL,@VAR_PRE_REMBMAX);

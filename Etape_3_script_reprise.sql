...
WHILE @VAR_INDIVIDUS_LIGNE_EN_COURS<=@VAR_TOTAL_INDIVIDUS--boucle par salariés
	...
	@VAR_CEMP=IND_CEMP,--récupération du CEMP
	@VAR_BULL_SIREN=IND_SIREN,--récupération du SIREN de la société
	@VAR_BULL_SALARIE=IND_MATRICULEHRS --récupération du matricule HRS
	...
	WHILE @VAR_MOIS_EXPORT<=12 --boucle par mois/salariés
		...
		SELECT  @VAR_BULL_DATEDEBUT=TRANSCO_PPAIE FROM ENTETEPAIE  WHERE MATRI=@VAR_CMATR AND TRANSCO_MOIS=@VAR_MOIS_EXPORT AND CEMP=@VAR_CEMP GROUP BY TRANSCO_PPAIE;--date de début du bulletin
		SELECT  @VAR_BULL_DATEFIN=TRANSCO_PPAIE_FIN FROM ENTETEPAIE  WHERE MATRI=@VAR_CMATR AND TRANSCO_MOIS=@VAR_MOIS_EXPORT AND CEMP=@VAR_CEMP GROUP BY TRANSCO_PPAIE_FIN;--date de fin du bulletin
...
WHILE @VAR_ENTETE_LIGNE_EN_COURS<=@VAR_TOTAL_ENTETE--boucle bulletin
	...
	SELECT 
	@VAR_CMATR=MATRI,--récupération du matricule en cours
	@VAR_CEMP=CEMP,--récupération du CEMP
	@VAR_BULL_DATEDEBUT=TRANSCO_PPAIE,--récupération de la date de début
	@VAR_BULL_DATEFIN=TRANSCO_PPAIE_FIN, --récupération de la date de début
	@VAR_BULL_BASE=BASE,--récupération base rubrique
	@VAR_BULL_NRUB=NRUB --récupération du code rubrique
	FROM ENTETEPAIE
	WHERE @VAR_ENTETE_LIGNE_EN_COURS=ENTETEPAIE_LIGNE;

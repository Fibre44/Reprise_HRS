...
@VAR_PCI_DATEANCSIRET date,
@VAR_PAR_PLE_PARTAGE char(1),
@VAR_NOM_TABLE char(14),
@VAR_PCI_STATUTBOETH char(1);

SET @VAR_NOM_TABLE='CONTRATTRAVAIL';
...
WHILE @VAR_CONTRAT_EN_COURS<=@VAR_TOTAL_CONTRAT --boucle par contrat
	...
	SELECT TOP(1) @VAR_PCI_TYPECONTRAT=CTNAT FROM VALIDCONTRAT WHERE GCREF=@VAR_GCREF AND MATRI=@VAR_CMATR ORDER BY DVCTR DESC; --récupération du type contrat
		...
		BEGIN
			...
			SET @VAR_PCI_TYPECDD='';
		END
	SELECT @VAR_PCI_DEBUTCONTRAT=CAST(DENTR AS datetime),@VAR_PCI_FINCONTRAT=CAST(DDEP AS datetime),@VAR_PCI_SALARIEREMPL_HRU=MATRI  FROM GENCONTRAT WHERE GENCONTRAT_LIGNE=@VAR_CONTRAT_EN_COURS; --récupération datetime d'entrée
	...
	SELECT TOP(1) @VAR_PCI_STATUTBOETH=CTA50 FROM VALIDCONTRAT WHERE GCREF=@VAR_GCREF AND MATRI=@VAR_CMATR ORDER BY DVCTR DESC; --récupération du statut BOETH
	...
	SET @VAR_PCI_DATEANCSIRET='01/01/2021';
	...
	@VAR_PCI_TYPEINSEE,@VAR_PCI_CODEINSEE,@VAR_PCI_ANCIENNUM,@VAR_PCI_PASEXCLU,@VAR_PCI_DNACODUNITEHOR,@VAR_PCI_SALARIEREMPL,@VAR_PCI_LIBELLEEMPLOI,@VAR_PCI_MOTIFCTINT,@VAR_PCI_MOTIFSORTIE,@VAR_PCI_SIRETANCIEN,@VAR_NOM_TABLE,@VAR_PCI_DATEANCSIRET,@VAR_PCI_STATUTBOETH);

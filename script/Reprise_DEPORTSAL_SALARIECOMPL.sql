...
UPDATE VENTILATION_INDIVIDUS SET IND_STATUS='Etape DEPORTSAL', IND_REPRISE_HR_SPRINT_DEPORTSAL='X' WHERE IND_MATRICULEHRU=@VAR_CMATR AND IND_CEMP=@VAR_CEMP; --mise � jour du statuts
...
--SALARIECOMPL gestion des dates

DECLARE 

@VAR_PSA_SIREN char(9),
@VAR_PSA_SALARIE char(10),
@VAR_PSA_DATEANCIENNETE datetime,
@VAR_PSZ_ANCIENCOLLEGE datetime,
@VAR_PSZ_ANCIENGROUPE datetime,
@VAR_NOM_TABLE char(13);

SET @VAR_NOM_TABLE='SALARIESCOMPL';
DECLARE SALARIESCOMPL_cursor CURSOR FOR
SELECT PSA_SIREN,PSA_SALARIE,PSA_DATEANCIENNETE FROM HR_SPRINT_SALARIES;

OPEN SALARIESCOMPL_cursor
FETCH NEXT FROM SALARIESCOMPL_cursor
INTO @VAR_PSA_SIREN,@VAR_PSA_SALARIE,@VAR_PSA_DATEANCIENNETE;

WHILE @@FETCH_STATUS = 0

BEGIN

    SET @VAR_PSZ_ANCIENCOLLEGE=@VAR_PSA_DATEANCIENNETE;
    SET @VAR_PSZ_ANCIENGROUPE=@VAR_PSA_DATEANCIENNETE;

    --si on a People doc la table SALARIECOMPL est déjà alimentée dans le cas contraire il faudra faire un INSERT

    IF (SELECT COUNT(*) FROM HR_SPRINT_SALARIECOMPL WHERE PSZ_SIREN=@VAR_PSA_SIREN AND PSZ_SALARIE=@VAR_PSA_SALARIE )=1 -- alors il existe un enregistrement

    BEGIN        
        UPDATE HR_SPRINT_SALARIECOMPL SET PSZ_ANCIENCOLLEGE=@VAR_PSZ_ANCIENCOLLEGE, PSZ_ANCIENGROUPE=@VAR_PSZ_ANCIENGROUPE WHERE PSZ_SIREN=@VAR_PSA_SIREN AND PSZ_SALARIE=@VAR_PSA_SALARIE;
    END
    ...
    BEGIN
        INSERT INTO HR_SPRINT_SALARIECOMPL
        VALUES (@VAR_PSA_SIREN,@VAR_PSA_SALARIE,'',@VAR_PSZ_ANCIENCOLLEGE,@VAR_PSZ_ANCIENGROUPE,@VAR_NOM_TABLE)
    END
...
CLOSE SALARIESCOMPL_cursor;  
DEALLOCATE SALARIESCOMPL_cursor;  
GO  

...
@VAR_PRE_DATEDEBUT date,
@VAR_PRE_DATEFIN date,
...
SET @VAR_NOM_TABLE='RETENUESALAIRE';
...
IF @VAR_PRE_ORDRE=0
SET @VAR_PRE_ORDRE=1;
SELECT @VAR_PRE_DATEDEBUT=CAST(CONCAT(PENOT,'01') AS date) FROM SAISIEARRET WHERE SAISIEARRET_LIGNE=@VAR_SAISIEARRET_EN_COURS; --récupération de la date de début de la saisie
...
VALUES (@VAR_PRE_SIREN,@VAR_PRE_SALARIE,@VAR_PRE_ORDRE,@VAR_PRE_LIBELLE,@VAR_PRE_DATEDEBUT,@VAR_PRE_DATEFIN,@VAR_PRE_MONTANTTOT,@VAR_PRE_ACTIF,@VAR_PRE_RETENUESAL,@VAR_PRE_REMBMAX,@VAR_PRE_MONTANTMENS,@VAR_PRE_NBMOIS,@VAR_NOM_TABLE);

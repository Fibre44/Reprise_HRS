...
@VAR_TYPE_VALEUR_HRU int,--test si l'établissement existe d�j� dans la table de transco
...
SELECT @VAR_SOCIETE_CEMP=Employeur FROM EMPLOYEUR WHERE @VAR_SOCIETE_LIGNE=SOCIETE_LIGNE;--récupération de la soci�t� é traiter
SELECT @VAR_TRA_SIREN=Siren FROM EMPLOYEUR WHERE @VAR_SOCIETE_LIGNE=SOCIETE_LIGNE; --récupération du SIREN
	...
	SELECT @VAR_SOCIETE_ETABLISSEMENT=LEFT(Etablissement,4) FROM ETABLISSEMENT WHERE  ETABLISSEMENT_LIGNE=@VAR_ETABLISSEMENT_LIGNE;--récupération du code soci�t�
	Print 'Societe du code etb'+ RIGHT(@VAR_SOCIETE_ETABLISSEMENT,4)+'soci�t� en cours'+@VAR_SOCIETE_CEMP 

		IF @VAR_SOCIETE_ETABLISSEMENT=@VAR_SOCIETE_CEMP --Si le code soci�t� de l'établissement correspond é l'établissement en cours alors traitement

			BEGIN
			SELECT @VAR_ETABLISSEMENT_CODE_HRU=Etablissement FROM ETABLISSEMENT WHERE ETABLISSEMENT_LIGNE=@VAR_ETABLISSEMENT_LIGNE;
			SELECT @VAR_TYPE_VALEUR_HRU=COUNT(*) FROM TRANSCO WHERE TRA_VALEURHRU=@VAR_ETABLISSEMENT_CODE_HRU;--recherche si l'établissement a d�j� �t� trait�

			IF @VAR_TYPE_VALEUR_HRU=0 --Test si l'établissement a d�j� �t� import�
			...
			BEGIN
				...
				BEGIN
					...
					Print 'Soci�t� en cours '+@VAR_SOCIETE_CEMP+' l établissement '+@VAR_ETABLISSEMENT_CODE_HRU+' devient '+@VAR_ETABLISSEMENT_CODE_HRS-- Affiche du message
					SET @VAR_COMMENTAIRE='Soci�t� en cours '+@VAR_SOCIETE_CEMP+' l établissement '+@VAR_ETABLISSEMENT_CODE_HRU+' devient '+@VAR_ETABLISSEMENT_CODE_HRS;
			...
			ELSE --L'établissement existe d�j�

				BEGIN

				Print 'L établissement a d�j� �t� trait�'
				SET @VAR_COMMENTAIRE='L établissement a d�j� �t� trait�';

				END
			END

			ELSE--l'établissement ne fait pas partie de la soci�t� en cours

			BEGIN

			Print 'L établissement ne fait pas partie de la soci�t�'
			END
...
PRINT 'Cr�ation de l établissement '+@VAR_ET_LIBELLE

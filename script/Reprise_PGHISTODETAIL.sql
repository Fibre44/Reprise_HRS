...
--les données sont dans VALIDCONTRAT on ne reprendra que certaines zones libellé emploi, salaires de base,coefficient,horaire mensuel,type contrat
...
@VAR_PHD_NEWVALEUR_TYPECONTRAT varchar(35),
...
@VAR_VALEUR_TYPECONTRAT_ORIGINE varchar(35),
@VAR_VALEUR_TYPECONTRAT_SUIVANTE varchar(35),
@VAR_TYPECONTRAT_LIBELLE varchar(35),
@VAR_NUMERO_CONTRAT int,
...
WHILE @VAR_SALARIE_LIGNE_EN_COURS<=@VAR_VALIDCONTRAT_TOTAL_LIGNES--Boucle dans VALIDCONTRAT sur le salarié en cours
	...
	BEGIN
		...
		@VAR_PHD_TRAITEMENTOK,@VAR_PHD_CODTABL,@VAR_PHD_CODEPOP,@VAR_PHD_POPULATION,@VAR_NOM_TABLE,@VAR_PHD_CONTRATTRAV);
		...
		--Gestion du type contrat

		SELECT @VAR_VALEUR_TYPECONTRAT_ORIGINE=CTNAT FROM VALIDCONTRAT WHERE VALIDCONTRAT_LIGNE=@VAR_SALARIE_LIGNE_EN_COURS;--récupération de la valeur d'origine
		IF @VAR_VALEUR_TYPECONTRAT_ORIGINE='01'
			SET @VAR_TYPECONTRAT_LIBELLE='CDI';
		ELSE
			SET @VAR_TYPECONTRAT_LIBELLE='CCD';
		SET @VAR_PHD_NEWVALEUR_TYPECONTRAT=@VAR_TYPECONTRAT_LIBELLE;--récupération du nouveau salaire de base
		SET @VAR_PHD_PGTYPEHISTO='001';
		SET @VAR_PHD_PGINFOSMODIF='PCI_TYPECONTRAT';
		SET @VAR_PHD_TYPEVALEUR='T';
		SET @VAR_PHD_TABLETTE='PGTYPECONTRAT';
		SET @VAR_PHD_CODEPOP='';
		SET @VAR_PHD_POPULATION='';
		SET @VAR_PHD_CONTRATTRAV='1';

		SELECT @VAR_PHD_DATEAPPLIC=CAST(DVCTR AS datetime) FROM VALIDCONTRAT  WHERE VALIDCONTRAT_LIGNE=@VAR_SALARIE_LIGNE_EN_COURS;--récupération de la date du changement

		Print 'Initialisation du type contrat'+@VAR_PHD_NEWVALEUR_TYPECONTRAT+' avec la date '+CAST(@VAR_PHD_DATEAPPLIC AS varchar(255))


		INSERT INTO HR_SPRINT_PGHISTODETAIL
		VALUES (@VAR_PHD_SIREN,@VAR_PHD_SALARIE,@VAR_PHD_ETABLISSEMENT,@VAR_PHD_PGINFOSMODIF,@VAR_PHD_PGTYPEHISTO,@VAR_PHD_NEWVALEUR_TYPECONTRAT,@VAR_PHD_TYPEVALEUR,@VAR_PHD_TABLETTE,@VAR_PHD_PGTYPEINFOLS,@VAR_PHD_DATEAPPLIC,
		@VAR_PHD_TRAITEMENTOK,@VAR_PHD_CODTABL,@VAR_PHD_CODEPOP,@VAR_PHD_POPULATION,@VAR_NOM_TABLE,@VAR_PHD_CONTRATTRAV);
...
--Gestion du code contrat
		
	SELECT @VAR_VALEUR_TYPECONTRAT_ORIGINE=CTNAT FROM VALIDCONTRAT WHERE VALIDCONTRAT_LIGNE=@VAR_SALARIE_LIGNE_EN_COURS;--récupération de la valeur d'origine
	SELECT @VAR_VALEUR_TYPECONTRAT_SUIVANTE=CTNAT FROM VALIDCONTRAT WHERE VALIDCONTRAT_LIGNE=@VAR_SALARIE_LIGNE_SUIVANTE;--récupération de la valeur suivante

	Print 'Le type contrat d origine est '+@VAR_VALEUR_TYPECONTRAT_ORIGINE+' la valeur +1 est '+@VAR_VALEUR_TYPECONTRAT_SUIVANTE

	IF @VAR_VALEUR_TYPECONTRAT_ORIGINE<>@VAR_VALEUR_TYPECONTRAT_SUIVANTE

		BEGIN

		IF @VAR_VALEUR_TYPECONTRAT_SUIVANTE='01'
			SET @VAR_TYPECONTRAT_LIBELLE='CDI';
		ELSE
			SET @VAR_TYPECONTRAT_LIBELLE='CCD';
		SET @VAR_PHD_NEWVALEUR_TYPECONTRAT=@VAR_TYPECONTRAT_LIBELLE;--récupération du nouveau salaire de base
		...
		Print 'Insertion du nouveau contrat '+@VAR_PHD_NEWVALEUR_TYPECONTRAT+' à la date du '+CAST(@VAR_PHD_DATEAPPLIC AS varchar(255))

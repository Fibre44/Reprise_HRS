...
Reprise_HRS/script/Reprise_ABSENCESALARIE.sql
DENIS Nicolas Reprise CP N-2 V1
Latest commit 71da448 20 hours ago
History
1 contributor
380 lines (281 sloc) 14.2 KB
USE REPRISE_HRU
Go
...
PRINT 'Cr�ation d une absence ligne en cours '+CAST(@VAR_LIGNE_ABSENCES_EN_COURS AS VARCHAR(255))+'/'+CAST(@VAR_LIGNE_TOTAL_ABSENCES AS VARCHAR(255))
...
LEFT JOIN SALARIES ON PSA_SALARIE=IND_MATRICULEHRS AND PSA_SIREN=IND_SIREN;

OPEN SOLDE_CP_N2_cursor
FETCH NEXT FROM SOLDE_CP_N2_cursor
INTO @VAR_PCN_SIREN,@VAR_IND_MATRICULEHRU,@VAR_PCN_SALARIE;

WHILE @@FETCH_STATUS = 0

BEGIN

	SELECT @VAR_ACQUISN2=VLRES FROM COMPTEURS WHERE MATRI=@VAR_IND_MATRICULEHRU AND CDRG='CPD' AND CDSSR='300'--à revoir
	SELECT @VAR_PRISN2=VLRES FROM COMPTEURS WHERE MATRI=@VAR_IND_MATRICULEHRU AND CDRG='CPD' AND CDSSR='300'--à revoir

	IF 	@VAR_ACQUISN2-@VAR_PRISN2>0

	BEGIN--alors solde positif à reprendre
		SELECT @VAR_ORDRE_TEMPORAIRE=COUNT(*) FROM HR_SPRINT_ABSENCESALARIE WHERE PCN_SALARIE=@VAR_PCN_SALARIE;--compte le nombre d'absence du salarié déjà traité
		SET @VAR_PCN_ORDRE=@VAR_ORDRE_TEMPORAIRE+1; --incrémente le compteur
		SET @VAR_PCN_JOURS=@VAR_ACQUISN2-@VAR_PRISN2;--solde N-2
		SET @VAR_PCN_NBREMOIS=@VAR_PCN_JOURS/@VAR_PAR_NBRE_CP_ACQUIS;
		IF @VAR_PCN_NBREMOIS>12
		BEGIN
			SET @VAR_PCN_NBREMOIS=12;
		END
		SELECT @VAR_PCN_BASE=TAUXS FROM ENTETEPAIE WHERE MATRI=@VAR_IND_MATRICULEHRU AND NRUB=''--à revoir
	...
	ELSE
	BEGIN
		PRINT 'Passage au salarié suivant'
	END
